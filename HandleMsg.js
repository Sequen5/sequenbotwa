                    ...
                    for (let i of limit){
                        if(i.id === id){
                            ...
                            if (limits >= limitCount) {
                                found = true;
                                piyo.reply(from, `Maap Perintah BOT anda sudah mencapai batas, coba esok hari :)`, id)
                                return true;
                            }else{
                                limit
                                found = true;
...
case 'stiker':            
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                piyo.sendImageAsSticker(from, imageBase64 , 'Sequen')
                .then(() => {
...
break
case 'pornhub':
            if (args.length == 2) return piyo.reply(from, `Kirim perintah */pornhub [ |Teks1|Teks2 ]*, contoh *+pornhub |Sequen|Bot*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                piyo.reply(from, ind.wait(), id)
                const lpornhub = argz[1]
                ...
                await piyo.reply(from, `Wrong Format!\n[❗] Kirim perintah */pornhub [ |Teks1|Teks2 ]*, contoh *+pornhub |Sequen|Bot*`, id)
            }
            break   
...
case 'trumptweet':
                if (args.length == 0) return piyo.reply(from, `Kirim perintah /trumptweet [ Teks ], contoh +trumptweet Sequen`, id)
                 piyo.reply(from, ind.wait(), id)
...
case 'pollresult':
    if (!isRegistered) return piyo.reply(from , `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Sequen, untuk pendaftaran bisa menggunakan +register nama | Jenis Kelamin. Contoh: /register ${pushname}|cowo`, id)
...
case 'addv':
    if (!isRegistered) return piyo.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Piyobot, untuk pendaftaran bisa menggunakan +register nama | Jenis Kelamin. Contoh: /register ${pushname}|cowo`, id)
...
case 'bye':
        if (!isGroupMsg) return piyo.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
        if (!isGroupAdmins) return piyo.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
        piyo.sendText(from, 'Selamat Tinggal... ( ⇀‸↼‶ )').then(() => piyo.leaveGroup(groupId))
        break
case 'del':
        if (!isGroupAdmins) return piyo.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
...
case 'next':
                    ...
                    break
case 'send':
                     if (!isRegistered) return piyo.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Sequen Bot, untuk pendaftaran bisa menggunakan *+register* nama | no hp.    Contoh: +register |${pushname}|cewe/cowo`, id)
...
case 'quotemaker':               
            ...
            if (qmaker.length >= 3) {
                ...
                try {
                    ...
                    piyo.sendFileFromUrl(from, `${hasilqmaker}`, '', 'Nihh..', id)
                } catch {
                    piyo.reply('Yahh proses gagal, isinya sudah benar belum?..', id)
                }
            } else {
                piyo.reply(from, `Pemakaian ${prefix}quotemaker |isi quote|author|theme\n\ncontoh: ${prefix}quotemaker |aku sayang kamu|-sequen|random\n\nuntuk theme nya pakai random ya..`)
            }
            break
case 'loli':
                await piyo.reply(from, ind.wait(), id)
...
case 'jadwalbola':
            ...
            try {
                ...
                await piyo.sendText(from, xixixi, id)
                    ...
                    console.log(err)
                    await piyo.sendFileFromUrl(from, errorurl2, 'error.png', 'Maaf, Jadwal tidak ditemukan')
            }
            break
case 'daerah':
...
case 'infomotor':
        ...
        piyo.reply(from, 'Sabar oi lagi aku carii....\n\nKalo Kaga Muncul Ada yang salah', id)
        try {
...
case 'sspc':
                    ...
                    case 'listbacot':
                                    ...
                                    bacotanmu += '╚═〘 *Sequen Bot* 〙'
                                    await piyo.sendText(from, bacotanmu)
                                    break
                                        case 'addbacot':{
                                    if (!args.length >= 1) return piyo.reply(from, 'salah oi, +addbacot teksnya', id)  
        ...
        case 'chat':
            piyo.reply(from, `Untuk mengaktifkan Sequen Bot chat\n\n penggunannya\n${prefix}sequen on --mengaktifkan\n${prefix}sequen off --menonatifkan\n`, id)
            break
            break
        case 'sequen':
            if (isGroupMsg) 
			...
			if (args[0] == 'on') {
                ...
                piyo.reply(from, 'Mengaktifkan Sequen Bot!', id)
			} else if (args[0] == 'off') {
				let inxx = piyo.indexOf(chatId)
				piyo.splice(inxx, 1)
				fs.writeFileSync('./settings/piyo.json', JSON.stringify(piyo))
				piyo.reply(from, 'Menonaktifkan Sequen Bot!', id)
			} else {
				piyo.reply(from, `Untuk mengaktifkan Sequen Bot \n\nPenggunaan\n${prefix}sequen on --mengaktifkan\n${prefix}sequen off --nonaktifkan\n`, id)
            }
        }
			break    
		case 'simisimi':
...
case 'ban':
            ...
            if (args[0] == 'add') {
                ...
                piyo.reply(from, 'Mampus gua ban lu anjg wkwkwk!')
            } else
            if (args[0] == 'del') {
                let xnxx = banned.indexOf(args[1]+'@c.us')
                banned.splice(xnxx,1)
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                piyo.reply(from, 'Nih gua udh unbaned mhehe!')
            } else {
             for (let i = 0; i < mentionedJidList.length; i++) {
                banned.push(mentionedJidList[i])
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                piyo.reply(from, 'Mampus gua ban lu anjg wkwkwk!', id)
                }
            }
            break
         // AUTO STIKER
